...
const TASK_DONE = (domain, action) => {
  const index = domain.findIndex(item => item.get('id') === action.data.id);

  return domain.update(index, task => task.set('done', true));
};
...
const TASK_UNDONE = (domain, action) => {
  ...
  return domain.update(index, task => task.set('done', false));
};
